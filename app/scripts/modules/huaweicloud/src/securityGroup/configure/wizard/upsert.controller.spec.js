...
describe('Controller: huaweicloudCreateSecurityGroupCtrl', function() {
  // load the controller's module
  beforeEach(window.module(require('./upsert.controller').name));

  afterEach(HuaweiCloudProviderSettings.resetToOriginal);
  ...
  beforeEach(
    window.inject(function($controller, $rootScope, $q) {
      testSuite = this;
      HuaweiCloudProviderSettings.defaults.account = 'account1';
      ...
      this.createController = function(securityGroup) {
        this.ctrl = $controller('huaweicloudUpsertSecurityGroupController', {
          $scope: this.$scope,
  ...
  describe('initialized for create', function() {
    ...
    it('requests the list of accounts', function() {
      expect(AccountService.listAccounts).toHaveBeenCalledWith('huaweicloud');
    });
