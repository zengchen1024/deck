...
module.exports = angular
  .module('spinnaker.securityGroup.configure.huaweicloud.ports', [
    require('../../transformer').name,
    require('../../../common/validateType.directive').name,
    SECURITY_GROUP_READER,
  ])
  .controller('huaweicloudSecurityGroupRulesController', [
    '$scope',
    'huaweicloudSecurityGroupTransformer',
    'securityGroupReader',
    'cacheInitializer',
    function($scope, huaweicloudSecurityGroupTransformer, securityGroupReader, cacheInitializer) {
      this.infiniteScroll = {
        currentItems: 20,
      };
      ...
      this.addRule = function() {
        $scope.securityGroup.rules.push(huaweicloudSecurityGroupTransformer.constructNewIngressRule());
      };
      ...
      $scope.initializeSecurityGroups = function() {
        return securityGroupReader.getAllSecurityGroups().then(function(securityGroups) {
          ...
          if (account && region) {
            $scope.availableSecurityGroups = _.filter(securityGroups[account].huaweicloud[region]);
          } else {
